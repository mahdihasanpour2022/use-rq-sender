...
const requestHandler = (request) => {
    ...
    if (/\[[a-zA-Z]+\]/.test(URL) && request.pathParams) {
        ...
        const paramNamesArr = Array.from(URL.matchAll(/\[([a-zA-Z]+)\]/g), (m) => m[0]);
...
const errorHandler = (error) => {
    ...
    if (error.code === "ERR_NETWORK")
        console.error("vpn خود را قطع و از برقراری ارتباط اینترنت خود اطمینان حاصل نمایید.");
...
const refreshAuthLogic = async (failedRequest) => {
    ...
    const appURL = new URL(process.env.NEXT_PUBLIC_APP_URL).hostname;
    ...
    return await Axios.post(`${process.env.NEXT_PUBLIC_API_URL}/user/refresh-token`, {}, { headers: formData })
        .then(({ status, data }) => {
        if (status === 200) {
            if (data?.entity?.accessToken && data?.entity?.refreshToken) {
                cookies.set("kidzyLoginData", {
                    ...user,
